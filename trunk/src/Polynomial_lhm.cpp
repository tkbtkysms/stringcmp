...
Polynomial_lhm::Polynomial_lhm(){
  Polynomial();
}

void Polynomial_lhm::product(Polynomial_lhm pol2,int max_len){
  trie *tr = new trie();
...
void Polynomial_lhm::push_monomial(std::string term,int coe){
  int *ab = new int[5];
  for(int i = 0; i < 5;i++){
    ab[i] = 0;
  }
...
void Polynomial_lhm::push(Polynomial_lhm poly){
...
int Polynomial_lhm::getvalnum(int idx1,int idx2){
